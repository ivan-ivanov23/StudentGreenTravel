...
def fleg_assumptions(students: list, mode_of_transport: str, hub_uni: float, car: int = 0, taxi: int = 0, bus: int = 0):
    ...
    # Walk percentage is what's left from the 100%
...
def assign_scotland(bus_rail_students, p_car, p_taxi, p_bus):
    ...
    station_uni = 3.0
    bus_rail = fleg_assumptions(bus_rail_students, "bus/rail", station_uni, p_car, p_taxi, p_bus)
    return bus_rail

def assign_uk(bus_rail_students, plane_students, p_car, p_taxi, p_bus, p_car_plane, p_taxi_plane, p_bus_plane):
    ...
    airport_uni = 8.1
    bus_rail = fleg_assumptions(bus_rail_students, "bus/rail", station_uni, p_car, p_taxi, p_bus)
    plane = fleg_assumptions(plane_students, "plane", airport_uni, p_car_plane, p_taxi_plane, p_bus_plane)
